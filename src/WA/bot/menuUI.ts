...
export class MenuUIService {
  ...
  async sendMainMenu(client: any, to: string) {
    await client.sendMessage(to, {
      ...
      buttons: [
        { buttonId: 'pay', buttonText: { displayText: 'Make Payment' }, type: 1 },
  ...
  async sendPaymentMenu(client: any, to: string) {
    await client.sendMessage(to, {
      ...
      buttons: [
        { buttonId: 'cid', buttonText: { displayText: 'Enter Customer ID' }, type: 1 },
  ...
  async sendCustomerVerification(client: any, to: string, name: string, packageType: string) {
    await client.sendMessage(to, {
