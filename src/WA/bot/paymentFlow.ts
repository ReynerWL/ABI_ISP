...
export const paymentFlow = {
  initFlow: async (sock: WASocket, sender: string) => {
    ...
    await sock.sendMessage(sender, { text: 'Silakan masukkan Customer ID Anda:' });
  },
  ...
  handleResponse: async (sock: WASocket, sender: string, text: string) => {
    ...
    if (session.step === 'awaitingCustomerId') {
      try {
        ...
        if (res.data) {
          ...
          await sock.sendMessage(sender, { text: `Customer ID tidak ditemukan. Coba lagi.` });
        }
      } catch {
        await sock.sendMessage(sender, { text: `Terjadi kesalahan saat mengambil data.` });
      }
      return true;
    }
    ...
    if (session.step === 'awaitingConfirmation') {
      if (text.toLowerCase() === 'yes') {
        ...
        await sock.sendMessage(sender, { text: 'Silakan kirim gambar bukti pembayaran Anda.' });
      } else {
        sessions.delete(sender);
        await sock.sendMessage(sender, { text: 'Proses dibatalkan. Ketik "menu" untuk mulai lagi.' });
      }
      return true;
    }
...
handleImage: async function (sock: WASocket, sender: string, imageMessage: any) {
    const session = sessions.get(sender);
